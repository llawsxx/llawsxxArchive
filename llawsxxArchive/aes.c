...
int aes_encrypt_cbc_inplace(uint8_t data[], size_t data_len, const uint32_t key[], int keysize, const uint8_t iv[])
{
    ...
    for (idx = 0; idx < blocks; idx++) {
        uint8_t* buf = &data[idx * AES_BLOCK_SIZE];
        xor_buf(iv_buf, buf, AES_BLOCK_SIZE);
        aes_encrypt(buf, buf, key, keysize);
        memcpy(iv_buf, buf, AES_BLOCK_SIZE);
    }
    return(TRUE);
}
...
int aes_decrypt_cbc_inplace(uint8_t data[], size_t data_len, const uint32_t key[], int keysize, const uint8_t iv[])
{
    uint8_t iv_buf[AES_BLOCK_SIZE],temp_buf[AES_BLOCK_SIZE];
    size_t blocks, idx;
    ...
    for (idx = 0; idx < blocks; idx++) {
        uint8_t* buf = &data[idx * AES_BLOCK_SIZE];
        memcpy(temp_buf, buf, AES_BLOCK_SIZE);
        aes_decrypt(buf, buf, key, keysize);
        xor_buf(iv_buf, buf, AES_BLOCK_SIZE);
        memcpy(iv_buf, temp_buf, AES_BLOCK_SIZE);
    }
